...
uint32_t deal_asr_msg_by_cmd_id(sys_msg_asr_data_t *asr_msg, cmd_handle_t cmd_handle, uint16_t cmd_id)
{
    ...
    switch(cmd_id)
    {
        ...
        case 2://“抓取一号目标”
        {
            gpio_set_output_level_single(GPIO1,gpio_pin_4,1);   //输出高电平
            gpio_set_output_level_single(GPIO1,gpio_pin_5,0);   //输出高电平
            break;
        }
        case 3://“抓取二号目标”
        {
        	gpio_set_output_level_single(GPIO1,gpio_pin_4,1);   //输出高电平
			gpio_set_output_level_single(GPIO1,gpio_pin_5,1);   //输出高电平
            break;
        }
        case 4://“切换体感控制”
        {
